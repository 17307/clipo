...
Grab `Clipo-0.1.0.dmg` from the `build/` directory, drag `Clipo.app` into `/Applications`, and launch it. Grant Accessibility permission when prompted.
...
All hotkeys are rebindable in Settings → General.
...
- Add apps or regex patterns in Settings → Ignore to exclude specific sources.
